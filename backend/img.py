...
def encrypt_file (plain_text, key, block_size):
    ...
    while i < len(plain_text) :
        ...
        # print("Temp encrypted: ", temp_encrypted)
        # print("Int: ", int(temp_encrypted,16))
...
encrypted_image_matrix = pd.read_csv('encrypted_image_matrix.csv').values

decrypted_image_array = np.empty_like(encrypted_image_matrix, dtype='uint8')

for i in range(len(encrypted_image_matrix)):
    for j in range(len(encrypted_image_matrix[i])):
        d = decrypt_file(encrypted_image_matrix[i][j], key, block_size)
        # x trucate the x from the end of the string
        # extract only pixels from the string
        x = ''
        for c in d:
            if c.isdigit():
                x += c
